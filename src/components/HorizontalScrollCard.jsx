...
const HorizontalScrollCard = ({ data = [], heading, trending, media_type }) => {
    ...
    return (
        <div className="container mx-auto px-3 my-10 capitalize">
            ...
            <div className="relative ">
                <div ref={containerRef} className="grid grid-cols-[repeat(auto-fit,230px)] grid-flow-col gap-6 overflow-x-scroll overflow-hidden relative z-10 scroll-smooth transition-all scrollbar-none">
                    {
                        data.map((data, index) => {
                            return <Card key={data.id + "heading" + index} data={data} index={index + 1} trending={trending} media_type={media_type} />
                        })
                    }
                </div>
